...
"""" all indicators = [moving_average(df, 7), exponential_moving_average(df, 7), momentum(df, 7), rate_of_change(df, 7), average_true_range(df, 3),
                bollinger_bands(df, 3), ppsr(df, 3), stochastic_oscillator_k(df, 3), stochastic_oscillator_d(df, 3), trix(df, 3), average_directional_movement_index(),
                macd(), mass_index(df), vortex_indicator(df, 3), kst_oscillator(), relative_strength_index(df, 3), true_strength_index(), accumulation_distribution(df, 3)
      , chaikin_oscillator(df), money_flow_index(df, 3), on_balance_volume(df, 3), force_index(df, 3), ease_of_movement(df, 3), commodity_channel_index(df, 3)
      , coppock_curve(df, 3), keltner_channel(df, 3), ultimate_oscillator(df), donchian_channel(df, 3), standard_deviation(df, 3)]"""
indicators_check = [moving_average(df, 9), exponential_moving_average(df, 12), SuperTrend(df, 10, 3),macd(df,9,25)]
# print(moving_average(df, 9))
for indicator in indicators_check:
    df = pd.merge(df, indicator)

print(df.round(1).tail(5))
# Export Indicators Value To Excel File
df.index = np.arange(1, len(df) + 1)  # to make index start from 1
df.round(2).to_excel(f'{f}.xlsx', sheet_name="indicator Osama", index=True, index_label="No")
