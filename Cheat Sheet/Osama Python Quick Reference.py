...
simpsons.count('lisa')  # counts the number of instances عدد مرات وجود العنصر فى اليست
...
# Sort list of list by specified index (add By osama 11-2-2020)
test_list = [['Rash', 4, 28], ['Varsha', 2, 20], ['Nikhil', 1, 20], ['Akshat', 3, 21]]
# Method #1 : Using sort() + lambda
test_list.sort(key=lambda test_list: test_list[
    1])  # sort by a second index (رقم 1 يدل على الاندكس لو عوز تغير طبقا للاندكس التالت يبقى تغير 1 الى 2)
...
#             add By Osama Youssef 27-1-2020
# Python Input | How To Take Multiple Inputs in Python in one line
var1, var2 = input("Enter what has to be conveyed to the user here").split()  # for space-separated input
x, y = input("Enter First Name: "), input("Enter Last Name: ")
variable1, variable2, variable3 = map(int, input().split())
x, y = [x for x in input("Enter two value: ").split()]  # List Comprehension
# طريقة لادخال قيم ال list مع ملاحظة ان n عدد عناصر ال list
n = int(input())
list = []
for i in range(n):
    e = int(input())
    list.insert(i, e)
print(list)
# طريقة لادخال قيم list  واماكن هذة القيم حيث i مكان القيمة e
num.insert(i, e)
# ادخال قيم list &tuple &set  فى سطر واحد
variable1 = list(map(int, input().split()))
variable2 = tuple(map(int, input().split()))
variable3 = set(map(int, input().split()))
print(variable1)
print(variable2)
print(variable3)
# input list value by for loop
s = []
for x in range(0, 4):
    k = int(input("Please Enter Number: "))
    s.append(k)
print(s)
# insert value in spacial position in list
number = []
for x in range(0, 4):
    i, e = [int(x) for x in input('insert ').split()]
    number.insert(i, e)
print(number)
# طباعة اى list سواء طباعة القيم كلها فى سطر واحد او طباعة كل قيمة فى سطر
simpsons = ['homer', 'marge', 'bart']
for x in range(0, 3):
    print(simpsons[x])
print(simpsons)
...
where
 separator     هو الفاصل بين كل كلمة فى الجملة المراد عمل فصل لها
maxsplit هو اللى هتاخدة بين قوس لوحدة وتسيب الباقى 
مثال على دة لو maxsplit   مثلا 2    يبقى هيحط اول كلمة وتانى كلمة كل واحدة بين قوسين وباقى الكلام فى قوس لوحدهم
join() function in Python
string_name.join(iterable) 
string_name:دة الرمز اللى هيكون بين كل كلمتين من 
iterable:  المطلوب ربطة
'-'.join(1,2,3,4)  مثال
هتدينا   1-2-3-4join(iterable)

############################################################################
##classes
class Circle:  # لاحظ ان اول حرف capital
    pi = 3.14  # الثوابت فى الكلاس بمعنى القيم التى لا خلاف عليها وتستخدم فى كل الدوال داخل ال class

    def __init__(self, radius=1):  # المتغيرات التى سوف نستخدمها فى كل الدوال ويمكن ان نعطيها قيم مبدئية تستخدم فى حالة عدم ارسال متغيرات عند استدعلء الدالة
        self.radius = radius
...
my_circle = Circle(5)  # تم اخذ نسخة من ال   class وتم وضع لقيمة للمتغير radius  وهى 5
print(my_circle.radius)  # يمكن طباعة المتغيرات
print(my_circle.area)
print(my_circle.getCircumference())  # لاحظ ضرورة اضافة القوسين عند طباعة اى داله
